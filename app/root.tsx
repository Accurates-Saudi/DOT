...
async function loadRootCmsData(locale: string) {
  const { getPublicContentPayloadsByKeys } = await import(
  ...
  return getPublicContentPayloadsByKeys(keys);
}
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    ...
    const cmsDataPromise = loadRootCmsData(locale);

    const [session, overrides] = await Promise.all([
      getCmsAuthSession(request).catch(() => null),
      cmsDataPromise.catch(() => ({})),
    ]);

    cmsSession = session;
    cmsContentOverrides = overrides;
  } catch {
    cmsSession = null;
    cmsContentOverrides = {};
  }
